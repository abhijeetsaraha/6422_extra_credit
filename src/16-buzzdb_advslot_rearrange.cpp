...
public:
    ...
    // index where the latest deletion happened
    bool rearrange_tuples(size_t index)
    {
        Slot* slot_array = reinterpret_cast<Slot*>(page_data.get());
        for (size_t slot_itr=index; slot_itr<current_num_tuples-1; slot_itr++)
        {
            //rearrange tuples from index, slot_array[slot_itr] = slot_array[slot_itr+1]
            ...
            slot_array[slot_itr].length = slot_array[slot_itr + 1].length;
            std::memcpy(page_data.get() + offset, page_data.get()+slot_array[slot_itr+1].offset, slot_array[slot_itr+1].length);
        }
        //deleteTuple(current_num_tuples-1);
        slot_array[current_num_tuples-1].empty = true;
        return true;
    }
    ...
    bool addTuple(std::unique_ptr<Tuple> tuple) {
        ...
        if (slot_itr != current_num_tuples)
        {
            std::cout << "check if insertion happens in the middle after delete\n";
            std::cout << slot_itr << '\n';
        }
...
public:
    ...
    bool try_to_insert(int key, int value){
        ...
        for (size_t page_itr = 0; page_itr < num_pages; page_itr++) {
            ...
            std::cout << "Num_tuples: " << page->current_num_tuples << '\n';
        }
